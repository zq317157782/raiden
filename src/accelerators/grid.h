...
public:
	Grid(const std::vector<std::shared_ptr<Primitive>>& primitives,
		...
		_voxelPerUnit = (std::pow(_primitives.size(), 1.0f / 3.0f)*3) / diagonal[maxAxis];

		//计算每个轴下的体素个数
		for (int axis = 0; axis < 3; ++axis) {
			_voxelNum[axis] = (int)std::round(_voxelPerUnit * diagonal[axis]);
	...
	bool Intersect(const Ray& r, SurfaceInteraction* ref) const override {
		...
		for(int axis=0;axis<3;++axis){
			...
			if(r.d[axis]>=0){
				...
				nextCrossingT[axis]=hit+(VoxelToPos(pos[axis]+1,axis)-hitPoint[axis])/r.d[axis];
				step[axis]=1;//向正方向的标记
				out[axis]=_voxelNum[axis];//到第几个voxel结束
				deltaT[axis]=_widths[axis]/r.d[axis];
			}
			else{
				nextCrossingT[axis]=hit+(VoxelToPos(pos[axis],axis)-hitPoint[axis])/r.d[axis];
				step[axis]=1;//向负方向的标记
				out[axis]=-1;
				deltaT[axis]=-_widths[axis]/r.d[axis];
			}
