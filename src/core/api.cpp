...
std::shared_ptr<Medium> MakeMedium(const std::string &name,
	const ParamSet &paramSet, const Transform& mediumToWorld) {
	//默认参数
	...
	if (name == "homogeneous") {
		...
		int numItems = 0;
		const Float *data = paramSet.FindFloat("density", &numItems);
		if (data == nullptr) {
			LError << "No \"density\" values provided for heterogeneous medium?";
			return nullptr;
		}
		...
		if (numItems != nx*ny*nz) {
			LError << "GridDensityMedium has " << numItems << " density values; expected nx*ny*nz = " << nx*ny*nz;
			return nullptr;
		}
